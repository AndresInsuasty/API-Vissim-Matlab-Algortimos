...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    methods
     ...
     [greenT control] = obj.contrl_user_defined.calculateGreen(); 
...
%                 SensorsIn{i}{j} = 0;
         ...
         hybrid_c(i) = HybridController(obj.sp_from_controller,i,SensorsIn{i});
         clear SensorsIn
     end
...
%                 drawnow
                ...
                % Fix this obj.UTNSimulation.Controllers(:)
                for icll = 1:length(obj.UTNSimulation.Controllers(:))
                    xx = length(ID{icll}(:,end));
                    colas_actuales(xx*icll-xx+1:xx*icll,1) = ID{icll}(:,end);
                end
                % Put here The adaptative indexes
                obj.contrl_user_defined.ID1 = colas_actuales;
                ...
                %%% Se recalculan los greenTimes
                [greenT control]= obj.contrl_user_defined.calculateGreen();
                obj.contrl_user_defined = control;
                % Getting the signal plan calculated by ControllerUserDefined CLass
                 SpDB = obj.getSP(greenT);
                 % Actual signal Plan   
                 obj.sp_actual =  SpDB;     
                 %previous Signal plan used (Historic) DONE
                 obj.sp_old = SpDB;
